...
shell> for k in $(seq 1 20); do
    echo "Round ${r}; Key Seed ${k}";
    (time minisat-2.2.0 des_6t4_min_r${r}_s${k}.cnf) | tee minisat_r${r}_k${k}.result 2>&1;
  done;
done;
   \endverbatim
   is currently running.
...
shell> for k in $(seq 1 20); do
    echo "Round ${r}; Key Seed ${k}";
    (time minisat-2.2.0 des_6t4_canon_r${r}_s${k}.cnf) | tee minisat_r${r}_k${k}.result 2>&1;
  done;
done;
   \endverbatim
