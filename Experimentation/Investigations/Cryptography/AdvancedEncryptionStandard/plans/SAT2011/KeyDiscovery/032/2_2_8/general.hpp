// Matthew Gwynne, 20.7.2011 (Swansea)
...
/*!
  \file Investigations/Cryptography/AdvancedEncryptionStandard/plans/SAT2011/KeyDiscovery/032/2_2_8/general.hpp
  \brief Investigations into small-scale AES key discovery for AES with a 2x2 plaintext matrix and 8-bit field elements


  \todo Problem specification
  <ul>
  <li> We consider the small-scale AES with 2 row, 2 column, using the 8-bit
   field size for rounds 1 to 20. </li>
   <li> We denote this AES instance by aes(r,2,2,8) for r in 1,...,20. </li>
   <li> We investigate translations of the key discovery problem for
   aes(r,2,2,8) into SAT. </li>
   <li> aes(r,2,2,8) takes a 32-bit plaintext and 32-bit key and outputs a
   32-bit ciphertext. </li>
   <li> aes(r,2,2,8) applies the following operations:
    <ol>
...
K_(i,2,k) := S-box(K_(i-1,1,2)) + sum(K_(i-1,j,l),l,1,j)
   ...
   where
    <ul>
     ...
     <li> K_(i,j,k) is the 8-bit word in the j-th row, k-th column of the i-th
